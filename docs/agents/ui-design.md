...
- **Components:** `src/components/ui` (shadcn/Base UI), default variants; compose primitives before custom markup. Search upward for an existing wrapper before duplicating markup; hoist reusable UI to the highest relevant scope (segment `components/` → `src/components/`). Links: `render={<Link … />}`, not `asChild`. Do **not** hand-edit files under `src/components/ui` for project fixes — refresh with the shadcn CLI when upstream changes.
